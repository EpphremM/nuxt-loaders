...
if (process.argv.length < 3) {
    logErrorCli('No operation specified. Please use "nuxt-loaders add <loader>" or "nuxt-loaders remove <loader>"');
    process.exit(1);
}

const args = process.argv.slice(2);
if (args.length < 1) {
    logErrorCli('No loader specified. Please use "nuxt-loaders add <loader>" or "nuxt-loaders remove <loader>"');
    process.exit(1);
}
...
if (!templatesIndex) {
    logErrorCli("Could not find any loader templates. Please stay tuned for some.")
    process.exit(1);
}
