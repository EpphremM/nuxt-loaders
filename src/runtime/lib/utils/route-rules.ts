...
const checkLoaderNameValidity = (loaderName: string): { valid: boolean, reason?: string } => {
    // Validate loader component names: alphanumeric with optional hyphens/underscores
    const loaderNamePattern = /^[A-Za-z][A-Za-z0-9_-]*$/;

    if (!loaderNamePattern.test(loaderName)) {
        return { valid: false, reason: `Loader name '${loaderName}' is invalid. Must start with a letter and contain only alphanumeric characters, hyphens, or underscores.` }
    }

    return { valid: true }
}

const checkRoutePatternValidity = (route: string): { valid: boolean, reason?: string } => {
    ...
    if (!routePattern.test(route)) {
        return { valid: false, reason: "Route pattern is invalid. Must start with '/' and contain only alphanumeric characters, hyphens, underscores, or slashes." }
    }

    return { valid: true }
}

const checkRuleValidity = (route: string, loaderName: string): { valid: boolean, reason?: string } => {
...
export const validateLoaderRules = (routeRules: Record<string, string>) => {
    ...
    Object.keys(routeRules).forEach(route => {
        if (!route) return;
        if (!route.trim()) {
            logWarn(`Loader rule is invalid. Failed trying to parse '${route}'. Route is empty so it is skipped.`);
            return;
        };
        if (!routeRules[route] || (routeRules[route]?.trim().length || 0) < 1 || routeRules[route] == undefined) {
            logWarn(`Loader rule is invalid. Failed trying to parse '${route}: ${routeRules[route]}'. Loader name is invalid so it is skipped.`);
            return;
        };
        ...
        if (!validResult.valid) {
            logWarn(`Failed to parse rule '${normalizedRoute}'. Reason: ${validResult.reason}. Rule is skipped.`);
            return;
        }
...
export const getActiveLoader = (routeRules: Record<string, string>, path: string) => {
